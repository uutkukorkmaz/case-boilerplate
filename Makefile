...
define create-env-files
	...
	@if [ ! -f .env ]; then \
			...
			cp .env.example .env; \
			echo "CREATE DATABASE IF NOT EXISTS $${MYSQL_DATABASE};\nGRANT ALL PRIVILEGES ON $${MYSQL_DATABASE}.* TO '%'@'%';" > ./infrastructure/mysql/init.sql; \
	...
	@if [ ! -f ./src/.env ]; then \
			...
			sed -i '' "s/REDIS_PORT=6379/REDIS_PORT=$${REDIS_PORT}/g" ./src/.env; \
...
define app-health-check
	...
	@node -e 'require("http")\
		.get("http://localhost:$(NGINX_PORT)/health",response => (response.statusCode === 200 \
